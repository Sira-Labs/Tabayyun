...
- Python: `cd api && uv sync --extra dev && uv run pytest -q && uv run ruff check . && uv run mypy` (uv builds the core wheel from `../core/tabayyun-py`, and rebuilds it when a core source changes via `tool.uv.cache-keys` there; Rust toolchain required). Database tests run when `TABAYYUN_TEST_DATABASE_URL` is set (`make dev-infra` provides one); `make db-upgrade` / `make db-revision m="..."` for migrations; `make worker-dev` runs the job worker (or `TABAYYUN_INLINE_JOBS=true`).
